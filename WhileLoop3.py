...
while index < len(random_list): #are we at the end of the loop? if not, go back to while loop
    number = random_list[index] #get current number in our list
    count_list[number] = count_list[number] + 1 #increment our count for this number by one

    index = index + 1 #stops infnite loop---> when number index is>than the length of list
